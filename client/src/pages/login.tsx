...
function ProviderLogo({ provider }: { provider: CloudProvider }) {
  if (provider === 'aws') return <AWSLogo className="h-16 w-auto brightness-0 invert" />;
  if (provider === 'azure') return <AzureLogo className="h-16 w-auto" />;
  if (provider === 'gcp') return <GCPLogo className="h-16 w-auto brightness-0 invert" />;
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-6">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white mt-4">{config.name}</h1>
          <p className="text-gray-400 mt-1">FinOps Dashboard</p>
        </div>
        ...
        <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-6 backdrop-blur-sm">
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="bg-gray-900/50 px-3 text-gray-400">or sign in with email</span>
            </div>
          </div>
          ...
          <form onSubmit={handleFormLogin} className="space-y-4">
            ...
            <Button
              type="submit"
              variant="outline"
              className="w-full h-10 border-gray-700 text-gray-300 hover:text-white"
            >
              Sign In
            </Button>
          </form>
        ...
        <button
          onClick={handleBack}
          className="flex items-center gap-2 text-gray-400 hover:text-white mt-6 mx-auto transition-colors"
        >
